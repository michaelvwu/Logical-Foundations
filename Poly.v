...
Proof.
  intros X Y Z f p. destruct p as [m n]. reflexivity. Qed.
(** [] *)
...
Definition succ (n : nat) : nat := 
  fun X(f:X->X)(x:X)=>f(n X f x).
  
  (* simplified version of the one above.
  fun X f x => n X f (f x).
  *)
Example succ_1 : succ zero = one.
Proof. unfold succ. unfold one. unfold zero. reflexivity. Qed.

Example succ_2 : succ one = two.
Proof. unfold succ. unfold one. unfold two. reflexivity. Qed.
...
Proof. unfold plus. unfold zero. unfold one. reflexivity. Qed.

Example plus_2 : plus two three = plus three two.
Proof. unfold plus. unfold two. unfold three. unfold doit3times. reflexivity. Qed.

Example plus_3 :
  plus (plus two two) three = plus one (plus three three).
Proof. unfold plus. unfold two. unfold three. unfold one. unfold doit3times. reflexivity. Qed.
...
Proof. unfold mult. unfold zero. unfold plus. unfold three. reflexivity. Qed.

Example mult_3 : mult two three = plus three three.
Proof. unfold mult. unfold two. unfold three. unfold plus. unfold doit3times. reflexivity. Qed.
...
Definition exp (n m : nat) : nat. Admitted.
  (*fun X f x => m(X->X)(_:(X->X)->X->X)(_:X->X).*)
