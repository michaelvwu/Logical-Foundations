...
Proof.
  intros n m. apply tr_nm. Qed.
  
Example three_four: total_relation 3 4.
apply tr_nm. Qed.
(** [] *)
...
Proof.
  ...
  inversion H. Qed. (*inversion H0. rewrite H3 in H4.
  unfold not in H4. apply H4. reflexivity. Qed.*)
  
Example not_in_empty: ~ (empty_relation 3 4).
unfold not. intros. inversion H. Qed. (* inversion H0. rewrite H3 in H4.
unfold not in H4. apply H4. Admitted.*)
(** [] *)
...
Inductive Sigma: Type := aChar | bChar.

Definition my_reg_exp := App(Char aChar)(Star(Union(Char aChar)
(Char bChar))).
...
Lemma MStar' : forall T (ss : list (list T)) (re : reg_exp),
  ...
  fold app ss [] =~ Star re.
Proof.
 (* intros. induction ss. simpl. apply MStar0. simpl.
  apply MStarApp. apply H. apply MStar0.*)
  (*intros T s re H.
  rewrite <- (app_nil_r _ s).
  apply MStarApp.
  apply H.
...
Proof.
  ...
  inversion H. auto.
  auto. simpl. apply orb_true_iff. Admitted.
(** [] *)
