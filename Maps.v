...
Definition init_nat_env := t_empty 0.
Compute init_nat_env(Id "x").
...
Definition new_nat_env := t_update init_nat_env(Id "z") 1.
Compute new_nat_env(Id "x").
Compute new_nat_env(Id "z").
Compute init_nat_env(Id "z").

Compute init_nat_env.
Compute new_nat_env.
...
Proof.
  intros. unfold t_update. apply false_beq_id in H. rewrite H. 
  reflexivity. Qed.
(** [] *)
...
- intros x'. destruct x' as [n']. induction n'.
  ...
  - intros x'. destruct x' as [n']. induction n'.
    + unfold t_update. simpl. reflexivity.
    + unfold t_update. simpl. (* reflexivity. reflexivity.
Qed. *) Admitted.
(** [] *)
...
intros. apply functional_extensionality. destruct x. induction s.
- intros x'. destruct x' as [s']. induction s'.
  + reflexivity. 
  + unfold t_update. reflexivity.
- intros x'. destruct x' as [s']. induction s'.
    + unfold t_update. reflexivity.
    + unfold t_update.  (* destruct (beq_id (Id (S S')) (Id (S s'))) eqn: H.
...
Theorem update_same : forall X v x (m : partial_map X),
  ...
  update m x v = m.
Proof.
  intros X v x m H. unfold update. rewrite <- H. rewrite t_update_same.
  reflexivity.
Qed.
