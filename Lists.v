...
Theorem bag_theorem : forall (v t:nat) (s:bag),
  count v s = t -> count v (add v s) = S t.
Proof.
  intros v s t H.
  induction s as [|s'].
  - simpl. rewrite <- beq_nat_refl. rewrite <- H. reflexivity.
  - simpl. rewrite <- H. rewrite <- beq_nat_refl. reflexivity.
Qed.
...
Proof.
  intros l1 l2. induction l1 as [|n l1' IHl1'].
  - simpl. reflexivity.
  - simpl. rewrite -> IHl1'. destruct n. reflexivity.
...
(** Write down an interesting theorem [bag_count_sum] about bags
    ...
    [count]!) *)
Theorem bag_count_sum : forall (n:nat) (s:bag),
  leb (count n (sum s s)) (S (count n s)) = true.
Proof.
  intros n s. induction s as [|i s' IHs'].
  - simpl. reflexivity.
  - simpl. 
Admitted.
(** [] *)
...
Proof.
  intros d x v. simpl. rewrite <- beq_id_refl. reflexivity. Qed.
(** [] *)
